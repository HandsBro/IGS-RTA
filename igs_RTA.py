...
parser.add_argument('--threshold', type=int, default=90,
                    ...
                    help="Threshold value (default: 90).")
...
class Dataset:
    ...
    def get_last_task_labels(self):
        ...
        # last_task_labels2 = [
        #     (label.labelerId,label.label)
        #     for label in self.labels 
        #     if label.imageIdx == last_task_idx
        # ]
        # print(last_task_labels)
...
def update_data(data: Dataset, node, true_answer):
    ...
    for i in range(workers):
        accuracy = 1 / (1+np.exp(- alphas_gt[i]*betas_gt[node]))
        ...
        if (tasks <= EM_BATCH_SIZE): 
            ...
            # print(data.numImages*3 + i)
            data.labels[data.numImages*workers + i] = label
        # print(tasks, data.numImages, len(data.labels))
    
    data.numImages = (data.numImages + 1) % EM_BATCH_SIZE #as the pointer to access the latest task.
...
def unpack_parameters(x: np.ndarray, data: Dataset):
    data.alpha = x[:data.numLabelers].copy()  #deep copy in numpy
    data.beta = x[data.numLabelers:].copy()

def compute_log_probability(l: int, z: int, alpha: float, beta: float):
    exponent = np.exp(beta) * alpha
    if exponent>=700: exponent = 700
    if exponent<=-700: exponent = -700
        # print(f"exponent: {exponent}")
    
    if z == l:
        return -np.log1p(np.exp(-exponent))  # log(sigma)
    else:
        return -np.log1p(np.exp(exponent))   # log(1-sigma)
...
def compute_Q_function(data: Dataset):
    ...
    for label in data.labels:
        ...
        kernel = min(700, data.beta[j])
        exponent = np.exp(kernel) * data.alpha[i]
        if exponent>=700: exponent = 700
        if exponent<=-700: exponent = -700
...
def gradient_Q(data: Dataset):
    ...
    for label in data.labels:
        ...
        exponent = np.exp(data.beta[j]) * data.alpha[i]
        ...
        dQdAlpha[i] += term * np.exp(data.beta[j])
        dQdBeta[j] += term * data.alpha[i] * np.exp(data.beta[j])
...
def output_results(data: Dataset):
    ...
    # print("\nImage Parameters:")
    # for j, (beta, prob) in enumerate(zip(data.beta, data.probZ1)):
    #     print(f" Image {j}: Beta = {np.exp(beta):.4f}, GT: {betas_gt[j]} , P(Z=1) = {prob:.4f} ")
...
def select_node_entropy():
    ...
    for i in range(n):
        ...
        for j in range(workers):
            acc_list.append(1 / (1+np.exp( - alphas_es[j]*betas_es[i])))
...
def update_probability_em(data, node):
    ...
    errors = []
    for i in range(workers):
        errors.append(1 - 1 / (1+np.exp( - alphas_es[i]*beta))) ##extimated
...
def NIGS(data, target, threshold):
    ...
    # f=open('../Data/Amazon/AmazonNodeProbability.txt','r')
    p=json.load(f)
    f.close()
    ...
    while max_p<=threshold:
        ...
        # print(f"round {round}, selected node: {selected_node}")
        ...
        max_p=max(p)
        #progress.append(p.index(max_p))
    # output_results(data)
    return cost,p.index(max_p),progress
